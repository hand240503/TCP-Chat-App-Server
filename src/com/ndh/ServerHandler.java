...
class ServerHandler implements Runnable {
    ...
    private void handleCommand(String inputLine) {
        if ("CLOSE".equals(inputLine)) {
            log("CLOSE_REQUEST", "Nhận yêu cầu đóng kết nối từ client: " + clientSocket.getInetAddress() + ":" + clientSocket.getPort());
    ...
    private void handleLogin(JsonObject json) {
        if (json.has("data")) {
            ...
            if (dbConnect.login(username, password)) {
                ...
                sendAvatarToClient(user.getAvatar());
                sendMessage("LOGIN_SUCCESS", userJson);
    ...
    private void closeConnection() {
        try {
            ...
            log("CLOSE_CONNECTION", "Kết nối đã được đóng: " + clientSocket.getInetAddress() + ":" + clientSocket.getPort());
        } catch (IOException e) {
            log("ERROR", "Không thể đóng socket: " + e.getMessage());
        }
    }

    private void sendAvatarToClient(String filePath) {
